import React from "react";
import bestcollege from "../assets/gallery9.jpeg";

const Ayurvedi = () => {
  return (
    <div>
      <p className="text-center underline my-10 text-4xl text-[#072a33] font-extrabold ">
        Best Ayurveda College in Uttar Pradesh
      </p>
      <div className="flex items-center justify-between mb-10 lg:mx-40 lg:flex-row flex-col mx-10">
        <p className="lg:w-[600px] mx-2">
          {/* Shri Babu Singh Daddu Ji Ayurvedic Medical College & Hospital Founded
          by Shri Babu Singh Daddu Ji Educational Trust is situated at 8th
          milestone from fatehgarh city at farrukhabad kanpur state highway the
          college campus provides well accommodation facilities both ladies and
          gents seperately. The are herbal garden is being maintained which
          facilities the academic needs of student the huge library has in its
          store the medical books with around 7500 books and 350 titles. All the
          department are well equipped with chart models and as per the body
          norms. The aim of the institute is to make the institute a prime
          educational center of region so as to cater rural areas for the poor
          and humble people of india. A police chowki and nationalized bank is
          also setup in the college campus for safety purpose. The Institutes
          Building is a mordern marvel and a splendid examples of a well
          designed architectural structure. It has been planned in such a manner
          that it is easily accessible to any of the department. It also consist
          of a 1600 square feet green area of flora & fauna with various flowers
          & grass. There is a lift for the patient faculty and handicapped
          students. All rooms are well equipped with excellent furniture and
          multimedia facilities, which includes projector, while board,
          microphone and sound system. */}
          Lorem, ipsum dolor sit amet consectetur adipisicing elit. Natus deleniti quod deserunt. Suscipit ducimus voluptates ullam rerum voluptatum dolores deleniti similique provident odio odit, neque eveniet aut repellat velit assumenda itaque repellendus, magnam facilis explicabo tenetur? Aperiam rerum molestiae quisquam sunt inventore dicta illo quaerat sapiente numquam omnis corporis porro similique dolor, ipsam voluptatibus nobis voluptates ipsa perspiciatis fugiat pariatur nam facilis. Commodi, alias neque! Ut sit ullam minima magni animi cupiditate minus ipsa debitis recusandae voluptas nam aperiam corporis expedita officiis odit itaque dignissimos quaerat atque, at cumque soluta. Asperiores labore quaerat dolor minima. Neque consectetur ipsam debitis velit animi perferendis, earum consequuntur officia. Repellendus magni dicta, itaque corrupti dolorum atque minima quibusdam nisi esse ratione est nesciunt, officiis facere illo adipisci fuga. Voluptatem eveniet, odit minima vero quo mollitia commodi nemo, velit ratione, repellendus expedita. Autem, a fuga. Quaerat quidem voluptatibus inventore. Nisi tempore, molestias illum aperiam nihil eveniet saepe deserunt vel, incidunt quia vitae voluptas, provident vero quo repellat voluptatibus eaque. Minus doloremque unde ad blanditiis odit placeat, quidem quam libero, eos itaque quas totam rem. Laborum quibusdam minus perspiciatis nesciunt, aut omnis autem, asperiores quae molestias adipisci possimus iusto et vero dolore error culpa voluptatem quisquam.
        </p>
        <img
          src={bestcollege}
          className="rounded-lg"
          style={{ width: 500, height: 400 }}
        />
      </div>
    </div>
  );
};

export default Ayurvedi;
